...
class MailStore {
  ...
  /** Get mails in a specific folder */
  ...
  /**
   * Send a mail (adds to 'inbox' by default).
   * This is the main method to call from other components/files.
   *
   * Example:
   *   mailStore.send({
  ...
  /**
   * Compose and send (adds to 'sent' folder with read=true)
   */
  ...
  /** Move a mail to a different folder */
  moveTo(id: string, folder: MailFolder): void {
    this.mails = this.mails.map(m => m.id === id ? { ...m, folder } : m);
    this.emit();
  }

  /** Delete a mail (moves to trash, or removes permanently if already in trash) */
  remove(id: string): void {
    ...
    if (mail.folder === 'trash') {
      ...
      this.mails = this.mails.map(m => m.id === id ? { ...m, folder: 'trash' as MailFolder } : m);
    }
    this.emit();
  }
...
/** Global mail store singleton — import this to send/read mails from anywhere */
export const mailStore = new MailStore();

/** React hook — returns all mails reactively */
...
/** React hook — returns mails for a specific folder reactively */
